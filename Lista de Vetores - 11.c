...
float somaSalario(float *salario, float soma, int i, int qntd);
...
int main(void) {
	...
	do{
		...
		if(flag==0){
			...
			scanf("%f",&salario[i]);
			soma = soma + somaSalario(salario, soma, i, qntd);
			i++;
		}else{
			flag=0;
		}		
	...
	somaSalario (salario, soma, i, qntd);
...
float somaSalario(float *salario, float soma, int i, int qntd){
	...
	if(i>=qntd){
		printf("A média salarial paga pela empresa é de %f", soma/qntd);
	}else{
		soma=soma+salario[i];
	}
}
